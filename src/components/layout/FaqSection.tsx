...
const FAQ_BY_TAB: Record<string, FaqItem[]> = {
  ...
  home: [
    {
      ...
      answer: (
        <div className="space-y-2">
          <p>
            看响应里有没有「穿帮」的指纹和协议结构。例如 OpenAI 官方回包只会出现标准的 token 计数字段；如果中转站偷偷把你的请求转给其他开源模型再包装成 OpenAI 格式发回来，很多专有元数据和格式特征经常会露出来。
          </p>
          <p>
      ...
      answer: (
        <div className="space-y-2">
          ...
          <ul className="list-disc list-inside space-y-1.5 pl-1 text-neutral-300">
            <li><span className="text-[#6ee7b7] font-semibold bg-[#064e3b] px-2 py-0.5 rounded border border-[#059669]">85 ~ 100 分（绿色）</span>：高保真官方真品，所有签名与关键探针全部通过；</li>
            <li><span className="text-[#fcd34d] font-semibold bg-[#451a03] px-2 py-0.5 rounded border border-[#d97706]">50 ~ 84 分（黄色）</span>：存在可疑项（例如缺少官方私钥签名、伪造流式思维链或延迟异常）；</li>
            <li><span className="text-[#fda4af] font-semibold bg-[#4c0519] px-2 py-0.5 rounded border border-[#e11d48]">0 ~ 49 分（红色）</span>：证据确凿的严重降级或冒充（例如用廉价开源模型冒充顶尖旗舰）。</li>
          </ul>
        </div>
      ),
    },
    {
      question: '测完显示「0 个模型可用」，是接口挂了吗？',
      answer: (
        <div className="space-y-2">
          <p>常见原因分析：</p>
          <ol className="list-decimal list-inside space-y-1.5 pl-1 text-neutral-300">
            <li><strong>中转站关闭了公开模型列表</strong>：很多中转站管理员在后台隐藏了 <code className="text-[#cc785c] font-mono font-semibold">/v1/models</code> 路由。这种情况下，你依然可以在输入框直接手动填写模型名发起测试；</li>
            <li><strong>API Key 额度用尽或未授权</strong>：Key 本身失效或账户欠费；</li>
            <li><strong>本地网络与跨域限制</strong>：系统已内置透明代理与 SNI 握手兜底重试。</li>
          </ol>
        </div>
      ),
    },
    {
      ...
      answer: (
        <div className="space-y-2">
          ...
          <p>
            <strong>TPS (Tokens Per Second)</strong> 代表流式输出的吞吐速率。旗舰大模型（如 Claude 3.7 / GPT-5 / GPT-4o）通常在 30~80 tokens/s；轻量模型可达 100~300+ tokens/s。
          </p>
        </div>
      ),
    },
  ...
  quickping: [
    {
      ...
      answer: (
        <div className="space-y-2">
          ...
          <p>
            原生预置支持 OpenAI、Anthropic (Claude)、DeepSeek、xAI (Grok)、Google Gemini、Cerebras、硅基流动、OpenRouter 以及任何自定义接口 Base URL。
          </p>
        </div>
      ),
    },
  ...
  docs: [
    ...
    },
    {
      ...
      answer: (
        <p>
          不同客户端的规范不同。例如 NextChat 默认要求传入基础域名，而 LiteLLM / OpenAI SDK / Claude Code 需要标准的 <code className="text-[#cc785c] font-mono font-semibold">/v1</code> 结尾。导出模块已自动根据目标软件的标准进行了自适应格式化。
        </p>
      ),
    },
  ],
  ...
  return (
    <div className="space-y-5 pt-6 border-t border-[#2e2b27]">
      <div className="flex items-center gap-2.5">
        ...
        <h3 className="font-serif-display text-2xl font-semibold text-[#faf9f5] tracking-tight">
          常见问题 (FAQ)
        </h3>
      </div>
      ...
      <div className="space-y-3">
        {currentFaqList.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`rounded-2xl border transition-all duration-200 overflow-hidden smooth-card ${
                isOpen
                  ? 'border-[#cc785c]/60 bg-[#1b1a18] shadow-md'
            ...
            >
              <button
                ...
                className="w-full flex items-center justify-between p-5 text-left transition smooth-btn"
              >
                <span className="text-sm font-semibold text-[#faf9f5] pr-4 tracking-wide">
                  {item.question}
                </span>
              ...
              {isOpen && (
                <div className="px-5 pb-5 pt-3 text-sm text-neutral-300 leading-relaxed border-t border-[#2e2b27]/60 space-y-3 animate-in fade-in duration-200 font-normal">
                  {item.answer}

                  {item.summaryNote && (
                    <div className="mt-3 pt-3 border-t border-[#2e2b27]/40 text-xs text-[#faf9f5] font-medium bg-[#23211e] border border-[#44403c] p-3.5 rounded-xl tracking-wide shadow-sm">
                      💡 {item.summaryNote}
                    </div>
                  )}
                </div>
